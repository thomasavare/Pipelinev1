...
def recording(sr, device, blocksize, seconds):
    record = np.array([])
    with sd.InputStream(samplerate=sr, device=device,
        ...
        while record.shape[0] < sr * seconds:
            processed = q.get()
...
def recording_save(sr, device, blocksize, seconds, filename):
    record = np.array([])
...
def autorecord(sr, device, blocksize):
    ...
    with sd.InputStream(samplerate=sr, device=device,
                        channels=1, callback=callback, blocksize=blocksize):
        print('#' * 80)
        print('record start in 3 sec, Ctrl+C to stop manually, ')
        print('#' * 80)
        print(3)
        sleep(1)
        print(2)
        ...
        #       statistic indicator or apply fft to reduce noise if very fast
        # idea 2: computation of fft in another thread to not lose time before next block
        while var >= 0.000001 and record.shape[0] < sr * 9:
            processed = q.get()
            var = np.var(processed)
            print(var)
            if not len(record) > 0:
